...
export async function POST(req) {
    ...
    try {
        ...
        console.log(title, clientName, attendees, startDate, endDate, location, medicalKit)
        // validate start and end date
        if (new Date(startDate) > new Date(endDate)) {
            return Response.json({ error: "End Date must be greater than or equal to start date." }, { status: 400 });
        }

        const newEvent = new Event({ title, clientName, attendees, startDate, medicalKit, endDate, location, doctors, ...(note && { note }) });
        await newEvent.save();
